...
class AnalyticsMixin:
    ...
    def _question_mentions_label(self, q, label: str) -> bool:
        ...
        haystack = ' '.join(
            [str(q.get('prompt') or '')]
            + [str(text or '') for text in (q.get('choices') or {}).values()]
        )
        if needle.isupper() and len(needle) <= 8:
            return re.search(rf'\b{re.escape(needle)}\b', haystack) is not None
        return needle.lower() in haystack.lower()

    def _build_source_agreement_rows(self, questions=None) -> tuple[list[SourceAgreementRow], dict[int, SourceAgreementRow]]:
        ...
        for q in question_list:
            ...
            correct_signatures = {tuple(sorted(str(letter) for letter in item.get('correct', []))) for item in prompt_group}
            objective_code = str(q.get('objective_code') or '').strip()
            ...
            if len(prompt_sources) > 1 and len(correct_signatures) == 1:
                ...
                support_sources = sorted(objective_sources.get(objective_code) or topic_sources.get(topic) or {str(q.get('source_name') or 'Unknown source')})
    ...
    def _build_source_trust_rows(self, questions, source_agreement_rows: list[SourceAgreementRow]) -> tuple[list[SourceTrustRow], dict[str, SourceTrustRow]]:
        question_list = list(questions or self.master_questions)
    ...
    def _build_latent_weakness_rows(
        ...
        for q in list(questions or self.master_questions):
            ...
            if not history_events:
                continue
            guessed_correct = sum(1 for event in history_events if event.get('correct') and str(event.get('confidence') or '').strip() == 'Guessed')
            unsure_correct = sum(1 for event in history_events if event.get('correct') and str(event.get('confidence') or '').strip() == 'Unsure')
    ...
    def _build_transfer_strength_rows(
        ...
        for key, bucket in grouped.items():
            ...
            confidence = (float(bucket['confidence_total']) / max(int(bucket['confidence_seen']), 1)) if int(bucket['confidence_seen']) else 0.72
            score = (
                stability * 0.52
    ...
    def _build_abstraction_ladder_rows(
        ...
        for key, bucket in grouped.items():
            available_styles = sorted(bucket['available_styles'], key=self.STEM_STYLE_LADDER.index if all(style in self.STEM_STYLE_LADDER for style in bucket['available_styles']) else str)
            seen_styles = sorted(bucket['seen_styles'], key=self.STEM_STYLE_LADDER.index if all(style in self.STEM_STYLE_LADDER for style in bucket['seen_styles']) else str)
    ...
    def _build_error_boundary_rows(
        ...
        for key, bucket in grouped.items():
            ...
            strong_style, strong_attempts, strong_accuracy = max(style_rows, key=lambda item: (item[2], item[1], item[0]))
    ...
    def _build_question_freshness_map(self, history: list[QuestionHistoryEvent], records, questions=None) -> dict[int, float]:
        now = datetime.now()
        ...
        for q in list(questions or self.master_questions):
            ...
            if events:
                ...
                penalty += max(0, len([event for event in events if (now - self._parse_event_time(event)).days <= 7]) - 1) * 6.0
            else:
    ...
    def _build_phrasing_normalization_rows(
        ...
        for q in list(questions or self.master_questions):
            ...
            score = 100.0 - note_count * 9.0 - odd_punct * 12.0 - long_prompt * 0.8 - long_explanation * 0.18 - acronym_density * 2.5
            score = round(max(0.0, min(100.0, score)), 1)
            if score >= 82:
                label = 'Clean'
            elif score >= 62:
    ...
    def _build_burnout_risk_row(self, session_history: list[QuestionHistoryEvent] | list[dict[str, Any]]) -> BurnoutRiskRow:
        ...
        def fragile(group):
            return sum(1 for event in group if str(event.get('confidence') or '').strip() in ('Guessed', 'Unsure')) / max(len(group), 1)
        ...
        if score >= 62:
            ...
            label = 'Low'
        note = 'Recent answers show fatigue drift, so the engine will flatten difficulty and reduce bonus insertions.' if label != 'Low' else 'Recent answer quality looks stable.'
        return {
            'label': label,
            'score': score,
    ...
    def _build_momentum_profile(self, session_history: list[QuestionHistoryEvent] | list[dict[str, Any]], burnout_risk: BurnoutRiskRow | None = None) -> dict[str, Any]:
        events = [event for event in list(session_history or []) if event]
        ...
        fragile_rate = sum(1 for event in sample if str(event.get('confidence') or '').strip() in ('Guessed', 'Unsure')) / max(len(sample), 1)
    ...
    def _build_prerequisite_debt_rows(
        ...
        for key, bucket in grouped.items():
            objective_row = objective_mastery_map.get(str(bucket['unit']), {'mastery_score': 70.0}) if bucket['kind'] == 'Objective' else None
            stability = float(bucket['stability_total']) / max(int(bucket['stability_seen']), 1) if int(bucket['stability_seen']) else 0.0
            attempt_rate = int(bucket['attempted']) / max(int(bucket['available']), 1)
            mastery_score = float((objective_row or {}).get('mastery_score', max(0.0, min(100.0, stability * 0.78 + attempt_rate * 22.0))))
            ...
            dependent_units = sorted(unit_key.split('::', 1)[1] for unit_key in domain_units.get(str(bucket['domain']), set()) if unit_key != key)[:3]
    ...
    def _build_concept_half_life_rows(
        ...
        for q in list(questions or self.master_questions):
            ...
            if correct_events:
                bucket['confidence_total'] += sum(self._confidence_weight(event.get('confidence')) for event in correct_events) / len(correct_events)
            else:
    ...
    def _build_leverage_ranking_rows(
        ...
        for key, bucket in grouped.items():
            dependent_count = len({unit_key for unit_key, debt in prerequisite_debt_map.items() if unit_key != key and unit_key.split('::', 1)[0] == bucket['kind'] and debt['severity'] >= 45.0 and str(debt['unit']) != str(bucket['unit'])})
            related_objectives = len(bucket['related_objectives'] or domain_objectives.get(str(bucket['domain']), set()))
            leverage = float(bucket['available']) * 9.0 + float(bucket['scenario_hits']) * 6.0 + float(bucket['topic_mix']) * 5.0 + dependent_count * 7.5 + related_objectives * 4.0
    ...
    def _build_misconception_fingerprint_rows(
        ...
        for event in history:
            ...
            if family in ('Near-synonym / look-alike distractor', 'Technically true but not best', 'Order-of-operations trap'):
        ...
        for row in grouped.values():
            evidence = max(row['evidence'].items(), key=lambda item: item[1])[0] if row['evidence'] else 'recurring pattern'
            rows.append(
                {
    ...
    def _build_effort_efficiency_rows(
        ...
        for key, bucket in grouped.items():
            seen = max(int(bucket['seen']), 1)
            avg_time = float(bucket['time_total']) / max(int(bucket['time_seen']), 1) if int(bucket['time_seen']) else 0.0
            fragile_rate = int(bucket['fragile_correct']) / max(int(bucket['correct_total']), 1) if int(bucket['correct_total']) else 0.0
    ...
    def _build_reinforcement_distance_rows(
        ...
        for q in list(questions or self.master_questions):
            ...
            recommended_days = float(half_life_row.get('half_life_days', 4.0)) * 0.72 + min(int(rec.get('correct_streak', 0)), 4) * 0.7
    ...
    def _build_generalization_score_rows(
        ...
        for transfer_row in transfer_strength_rows:
            ...
            score = float(transfer_row['score']) * 0.52 + float(ladder.get('score', 60.0)) * 0.24 + float(robustness.get('score', transfer_row['score'])) * 0.24
    ...
    def _build_recognition_retrieval_rows(
        ...
        for q in list(questions or self.master_questions):
            ...
            bucket = grouped.setdefault(
                key,
                {'kind': kind, 'unit': unit, 'recognition_total': 0, 'recognition_correct': 0, 'retrieval_total': 0, 'retrieval_correct': 0},
            )
        ...
        for key, bucket in grouped.items():
            recognition_score = (int(bucket['recognition_correct']) / max(int(bucket['recognition_total']), 1)) * 100.0 if int(bucket['recognition_total']) else 0.0
            retrieval_score = (int(bucket['retrieval_correct']) / max(int(bucket['retrieval_total']), 1)) * 100.0 if int(bucket['retrieval_total']) else 0.0
    ...
    def _build_decision_latency_rows(
        ...
        for event in history:
            ...
            bucket = grouped.setdefault(
                key,
                {'kind': kind, 'unit': unit, 'productive_total': 0.0, 'productive_seen': 0, 'confusion_total': 0.0, 'confusion_seen': 0},
            )
            confidence = str(event.get('confidence') or '').strip()
        ...
        for key, bucket in grouped.items():
            productive = float(bucket['productive_total']) / max(int(bucket['productive_seen']), 1) if int(bucket['productive_seen']) else 0.0
            confusion = float(bucket['confusion_total']) / max(int(bucket['confusion_seen']), 1) if int(bucket['confusion_seen']) else 0.0
    ...
    def _build_cue_dependence_rows(
        ...
        for q in list(questions or self.master_questions):
            ...
            correct_choice_lengths = [len(str((q.get('choices') or {}).get(letter, '')).strip()) for letter in q.get('correct', [])]
            all_choice_lengths = [len(str(text or '').strip()) for text in (q.get('choices') or {}).values() if str(text or '').strip()]
            ...
            if correct_choice_lengths and all_choice_lengths:
                ...
                if max(correct_choice_lengths) >= longest:
                    cue_score += 10.0
            if re.search(r'\([A-Z0-9]{2,8}\)', ' '.join(str((q.get('choices') or {}).get(letter, '')) for letter in q.get('correct', []))):
                cue_score += 8.0
    ...
    def _build_retention_stress_rows(
        ...
        for q in list(questions or self.master_questions):
            ...
            half_life_days = float(half_life_row.get('half_life_days', 4.0))
            pressure = max(0.0, days_since - half_life_days) * 12.0 + max(0.0, float(robustness_row.get('score', 60.0)) - 70.0) * 0.25
            if pressure < 18.0:
                continue
    ...
    def _build_coverage_gap_rows(self, records, questions=None) -> list[CoverageGapRow]:
        ...
        for row in unit_rows.values():
            ...
            coverage = (attempted / max(available, 1)) * 100.0
            accuracy = (int(row['correct_events']) / max(int(row['attempt_events']), 1)) * 100.0 if int(row['attempt_events']) else 0.0
    ...
    def _build_confusion_pair_rows(self, history: list[QuestionHistoryEvent]) -> list[ConfusionPairRow]:
        ...
        for row in pair_rows.values():
            action = f"Drill the deciding difference between {row['left']} and {row['right']} until the clue is automatic."
            results.append(
                {
                    'pair': f"{row['left']} vs {row['right']}",
                    'left': row['left'],
            ...
            results.append(
                {
                    ...
                    'domains': ', '.join(sorted(row['domains'])[:3]),
                    'note': f"Clue '{clue}' needs stronger recall." if mastery_signal < 72.0 else f"Clue '{clue}' is becoming reliable.",
                }
            )
    ...
    def _build_concept_memory_state_rows(
        ...
        for key, bucket in grouped.items():
            ...
            confidence_quality = round((weighted_confidence / max(len(correct_events), 1)) * 100.0, 1) if correct_events else 0.0
            ...
            durability_signal = min(100.0, len(sure_correct) * 18.0 + transfer_evidence * 10.0 + min(28.0, days_since_correct * 4.0) - len(wrong_events) * 9.0 - len(fragile_correct) * 6.0)
            ...
            if evidence_count <= 0:
                ...
                state = 'recognizable'
            elif len(sure_correct) >= 3 and transfer_evidence >= 4 and durability_signal >= 72.0 and days_since_correct >= 1.0:
                state = 'durable'
    ...
    def _build_concept_clusters(
        ...
        for row in concept_rows.values():
            ...
            top_reason = max(row['miss_reason_counts'].items(), key=lambda item: item[1])[0] if row['miss_reason_counts'] else ''
            ...
            severity = round(row['misses'] * 4.4 + active_weak * 5.2 + due * 2.6 + (volatility / max(len(row['question_numbers']), 1)) * 0.12, 1)
    ...
    def _build_pass_prediction(
        ...
        source_trust = round(sum(row['score'] for row in source_agreement_rows) / len(source_agreement_rows) * 100.0, 1) if source_agreement_rows else 80.0
        gap_penalty = round(sum(row['severity'] for row in coverage_gaps[:3]) / max(len(coverage_gaps[:3]), 1), 1) if coverage_gaps else 0.0
        score = (
        ...
        if progress.get('attempted', 0) < 120:
            reasons.append(f"Only {progress.get('attempted', 0)} long-term answered questions are in the sample; confidence is still forming.")
        ...
        if stability_score < 70:
            reasons.append(f"Stability score is {stability_score}%, so knowledge is not holding consistently over time yet.")
        if source_trust < 84:
            reasons.append(f"Source agreement trust is {source_trust}%, so too much of your sample still relies on single-source-only items.")
        if coverage_gaps:
            top_gap = coverage_gaps[0]
            reasons.append(f"Coverage gap: {top_gap['kind']} {top_gap['unit']} is only {top_gap['attempted']}/{top_gap['available']} covered.")
        if not reasons:
    ...
    def _build_concept_anchor_notes(self, topic_rows: list[AnalyticsTopicRow], topic_history_map: dict[str, list[QuestionHistoryEvent]]) -> list[ConceptAnchorNoteRow]:
        notes = []
    ...
    def _build_wrong_answer_family_review(self, history: list[QuestionHistoryEvent]) -> list[WrongAnswerFamilyRow]:
        ...
        for event in history:
            ...
            row = families.setdefault(family, {'family': family, 'count': 0, 'domains': set(), 'topics': set(), 'examples': set()})
            row['count'] += 1
            row['domains'].add(str(event.get('domain') or 'Unsorted'))
            for topic in event.get('topics') or []:
    ...
    def _build_analytics_payload(self, source=None) -> AnalyticsPayload:
        ...
        active_questions = [q for q in (source or self.questions) if not is_suspended(records.get(self._question_key(q), {}))]
        ...
        recent = answered[-50:]
        recent_acc = round(sum(1 for q in recent if self._question_correct(q)) / len(recent) * 100, 1) if recent else 0.0
        history = [event for event in self._recent_history(28) if not is_suspended(records.get(str(event.get('question_number')), {}))]
        ...
        for q in questions:
            ...
            d = domain_stats.setdefault(domain, {'total': 0, 'answered': 0, 'correct': 0, 'wrong': 0, 'flagged': 0, 'issues': 0})
            d['total'] += 1
        ...
        for q in self.master_questions:
            ...
            if not rec or is_suspended(rec):
                continue
            domain = q.get('domain') or 'Unsorted'
            d = progress_domain_stats.setdefault(domain, {'progress_attempted': 0, 'progress_wrong': 0, 'progress_due': 0, 'progress_flagged': 0, 'progress_active_weak': 0, 'progress_recovered': 0, 'confidence_score': 0.0, 'confidence_seen': 0})
            attempts = int(rec.get('attempts', 0))
            ...
            for topic in q.get('topics', []):
                ...
                if not topic:
                    continue
                t = progress_topic_stats.setdefault(topic, {'progress_attempted': 0, 'progress_wrong': 0, 'progress_due': 0, 'progress_active_weak': 0, 'progress_recovered': 0, 'confidence_score': 0.0, 'confidence_seen': 0})
        ...
        source_trust_rows, source_trust_map = self._build_source_trust_rows(self.master_questions, source_agreement_rows)
        coverage_gaps = self._build_coverage_gap_rows(records, self.master_questions)
        coverage_gap_map = self._coverage_gap_priority_map(coverage_gaps)
        knowledge_trace_rows, knowledge_trace_map = self._build_knowledge_trace_rows(question_history_map, self.master_questions)
        concept_memory_state_rows, _concept_memory_state_map = self._build_concept_memory_state_rows(question_history_map, self.master_questions)
        wrong_answer_memory_rows, _wrong_answer_memory_map = self._build_wrong_answer_memory_rows(history, self.master_questions)
        confusion_pairs = self._build_confusion_pair_rows(history)
        interference_map_rows = self._build_interference_map_rows(history)
        confidence_compression_rows, confidence_compression_map = self._build_confidence_compression_rows(records, question_stability, question_history_map, self.master_questions)
        abstraction_ladder_rows, abstraction_ladder_map = self._build_abstraction_ladder_rows(records, question_stability, question_history_map, self.master_questions)
        recognition_retrieval_rows, recognition_retrieval_map = self._build_recognition_retrieval_rows(question_history_map, self.master_questions)
        compression_point_rows, compression_point_map = self._build_compression_point_rows(recognition_retrieval_map, abstraction_ladder_map)
        ...
        counterfactual_distractor_rows, counterfactual_pressure_map = self._build_counterfactual_distractor_rows(history, self.master_questions)
        contrast_rule_rows, contrast_pressure_map = self._build_contrast_rule_rows(counterfactual_distractor_rows, confusion_pairs, self.master_questions)
        objective_mastery_rows, objective_mastery_map = self._build_objective_mastery_rows(records, question_stability, question_history_map, source_agreement_map, self.master_questions)
        prerequisite_debt_rows, prerequisite_debt_map = self._build_prerequisite_debt_rows(records, question_stability, coverage_gaps, objective_mastery_map, error_boundary_map, counterfactual_pressure_map, self.master_questions)
        concept_half_life_rows, concept_half_life_map = self._build_concept_half_life_rows(records, question_stability, question_history_map, self.master_questions)
        leverage_ranking_rows, leverage_map = self._build_leverage_ranking_rows(records, prerequisite_debt_map, self.master_questions)
        blind_spot_rows, blind_spot_map = self._build_blind_spot_inference_rows(coverage_gaps, prerequisite_debt_map, leverage_map, self.master_questions)
        difficulty_rows, difficulty_map = self._build_difficulty_calibration_rows(records, question_stability, question_history_map, source_agreement_map, self.master_questions)
        phrasing_rows, phrasing_map = self._build_phrasing_normalization_rows(self.master_questions)
        effort_efficiency_rows, effort_efficiency_map = self._build_effort_efficiency_rows(history, self.master_questions)
        misconception_fingerprints, misconception_pressure_map = self._build_misconception_fingerprint_rows(history, self.master_questions)
        ...
        for name, d in domain_stats.items():
            ...
            progress_bits = progress_domain_stats.get(name, {'progress_attempted': 0, 'progress_wrong': 0, 'progress_due': 0, 'progress_flagged': 0, 'progress_active_weak': 0, 'progress_recovered': 0, 'confidence_score': 0.0, 'confidence_seen': 0})
            heat = round((100 - acc) * 0.65 + max(0, 70 - coverage) * 0.35 + d['wrong'] * 1.5 + progress_bits['progress_wrong'] * 0.8 + progress_bits['progress_active_weak'] * 2.4 + progress_bits['progress_due'] * 1.8, 1)
            confidence_avg = (progress_bits['confidence_score'] / progress_bits['confidence_seen']) if progress_bits['confidence_seen'] else 0.75
            stability_values = [
                question_stability.get(int(q.get('question_number') or 0), 0.0)
                for q in self.master_questions
                if (q.get('domain') or 'Unsorted') == name and records.get(self._question_key(q), {}) and not is_suspended(records.get(self._question_key(q), {}))
            ]
            stability = round(sum(stability_values) / len(stability_values), 1) if stability_values else 0.0
            readiness = round(max(0.0, min(100.0, acc * 0.4 + coverage * 0.16 + confidence_avg * 16 + stability * 0.18 + max(0, progress_bits['progress_recovered']) * 1.4 - progress_bits['progress_active_weak'] * 4.0 - progress_bits['progress_due'] * 2.0)), 1)
            trend = round(self._trend_delta_for_group(domain_history_map.get(name, [])), 1)
            domain_rows.append({'domain': name, **d, **progress_bits, 'accuracy': acc, 'coverage': coverage, 'heat': heat, 'readiness': readiness, 'stability': stability, 'trend': trend})
        ...
        for name in sorted(set(topic_stats) | set(progress_topic_stats)):
            ...
            progress_bits = progress_topic_stats.get(name, {'progress_attempted': 0, 'progress_wrong': 0, 'progress_due': 0, 'progress_active_weak': 0, 'progress_recovered': 0, 'confidence_score': 0.0, 'confidence_seen': 0})
            ...
            stability_values = [
                question_stability.get(int(q.get('question_number') or 0), 0.0)
                for q in self.master_questions
                if name in [str(topic).strip() for topic in q.get('topics', [])] and records.get(self._question_key(q), {}) and not is_suspended(records.get(self._question_key(q), {}))
            ]
            stability = round(sum(stability_values) / len(stability_values), 1) if stability_values else 0.0
            readiness = round(max(0.0, min(100.0, acc * 0.42 + confidence_avg * 20 + stability * 0.22 - progress_bits['progress_active_weak'] * 4.0 - progress_bits['progress_due'] * 2.2 + progress_bits['progress_recovered'] * 1.2)), 1)
            trend = round(self._trend_delta_for_group(topic_history_map.get(name, [])), 1)
            topic_rows.append({'topic': name, **t, **progress_bits, 'accuracy': acc, 'readiness': readiness, 'stability': stability, 'trend': trend})
        topic_rows.sort(key=lambda x: (x['progress_active_weak'], x['progress_due'], x['progress_wrong'], -x['accuracy']), reverse=True)
        ...
        volatile_rows.sort(key=lambda row: (row['score'], row['flips'], row['attempts'], row['question_number']), reverse=True)
        ...
        if history:
            ...
            narrowed_count = sum(1 for event in history if str(event.get('miss_reason') or '').strip() == 'Narrowed to two')
            changed_count = sum(1 for event in history if str(event.get('miss_reason') or '').strip() == 'Changed answer')
            guessed_correct = sum(1 for event in history if event.get('correct') and str(event.get('confidence') or '').strip() == 'Guessed')
            unsure_correct = sum(1 for event in history if event.get('correct') and str(event.get('confidence') or '').strip() == 'Unsure')
            ...
            for event in history:
                ...
                if not question:
                    continue
                choice_lengths = [len(str(text or '').strip()) for text in question.get('choices', {}).values() if str(text or '').strip()]
                if not choice_lengths:
                    continue
                longest = max(choice_lengths)
                selected_texts = [str(text or '').strip() for text in event.get('selected_texts') or [] if str(text or '').strip()]
                if any(len(text) >= longest for text in selected_texts):
                    longest_choice_wrong += 1
            if total_wrong and misread_count / total_wrong >= 0.3:
                anti_patterns.append(f"Misread trap: {misread_count}/{total_wrong} recent misses were tagged as misreads.")
            if total_wrong and narrowed_count / total_wrong >= 0.25:
                anti_patterns.append(f"Two-answer trap: {narrowed_count}/{total_wrong} recent misses were narrowed to two.")
            if total_wrong and changed_count / total_wrong >= 0.2:
                anti_patterns.append(f"Second-guessing trap: {changed_count}/{total_wrong} recent misses came from changing your answer.")
            if total_wrong >= 3 and longest_choice_wrong / total_wrong >= 0.35:
                anti_patterns.append(f"Choice-length bias: {longest_choice_wrong}/{total_wrong} recent misses came from picking the longest answer.")
            if total_wrong >= 3 and fast_wrong_count / total_wrong >= 0.35:
                anti_patterns.append(f"Speed-risk pattern: {fast_wrong_count}/{total_wrong} recent misses were locked in under 7 seconds.")
            if guessed_correct >= 4:
                anti_patterns.append(f"False mastery risk: {guessed_correct} recent correct answers were still tagged as guesses.")
            if unsure_correct >= 5:
            ...
            if sure_group and sure_group['attempts'] >= 6 and sure_group['accuracy'] < 70:
                anti_patterns.append(f"Calibration drift: 'Sure' answers are only {sure_group['accuracy']}% accurate right now.")
            for trap_word, count in sorted(trap_word_counts.items(), key=lambda item: (item[1], item[0]), reverse=True)[:3]:
                if count >= 2:
        ...
        topic_names = sorted({str(topic).strip() for q in self.master_questions for topic in q.get('topics', []) if str(topic).strip()})
        for topic in topic_names:
            counts = {'New': 0, 'Active weak': 0, 'Recovered': 0, 'Recovered - due': 0, 'Due review': 0, 'Mastered': 0, 'In progress': 0, 'Flagged': 0}
            for q in self.master_questions:
        ...
        mastery_rows.sort(key=lambda row: (row['active_weak'], row['due'], -row['mastered'], row['topic']), reverse=True)
        ...
        latent_weakness_rows = self._build_latent_weakness_rows(records, question_stability, question_history_map, source_agreement_map, source_trust_map, self.master_questions)
        transfer_strength_rows, transfer_strength_map = self._build_transfer_strength_rows(records, question_stability, self.master_questions)
        robustness_rows, robustness_map = self._build_robustness_score_rows(transfer_strength_rows, abstraction_ladder_map, concept_half_life_map)
        generalization_rows, generalization_map = self._build_generalization_score_rows(transfer_strength_rows, abstraction_ladder_map, robustness_map)
        reinforcement_distance_rows, reinforcement_distance_map = self._build_reinforcement_distance_rows(records, question_stability, concept_half_life_map, self.master_questions)
        synthesis_check_rows, synthesis_check_map = self._build_synthesis_check_rows(objective_mastery_map, self.master_questions)
        cue_dependence_rows, cue_dependence_map = self._build_cue_dependence_rows(question_history_map, recognition_retrieval_map, phrasing_map, self.master_questions)
        delayed_probe_rows, delayed_probe_map = self._build_delayed_probe_rows(records, concept_half_life_map, self.master_questions)
        counterexample_training_rows, counterexample_training_map = self._build_counterexample_training_rows(counterfactual_distractor_rows, contrast_pressure_map, self.master_questions)
        failure_mode_rows, failure_mode_map = self._build_failure_mode_rows(history, self.master_questions)
        expected_learning_gain_rows, expected_learning_gain_map = self._build_expected_learning_gain_rows(records, knowledge_trace_map, leverage_map, difficulty_map, self.master_questions)
        retention_stress_rows, retention_stress_map = self._build_retention_stress_rows(records, concept_half_life_map, robustness_map, self.master_questions)
        concept_state_rows, concept_state_map = self._build_concept_state_rows(knowledge_trace_map, generalization_map, robustness_map, concept_half_life_map)
        attempted_stability_values = [score for score in question_stability.values() if score > 0]
        stability_score = round(sum(attempted_stability_values) / len(attempted_stability_values), 1) if attempted_stability_values else 0.0
        recs: list[str] = []
        if progress['due']:
        ...
        if progress['recovered']:
            recs.append(f"{progress['recovered']} previously missed questions are now recovered and moved out of the weak bucket.")
        if decision_quality:
            recs.append(f"Decision quality is {decision_quality}%. Keep pushing 'Sure' accuracy higher than guessed wins.")
        if volatile_rows:
            hottest = volatile_rows[0]
            recs.append(f"Q{hottest['question_number']} is highly volatile at {hottest['score']} with {hottest['flips']} right/wrong flips. Recheck the underlying concept, not just the answer letter.")
        if concept_clusters:
            cluster = concept_clusters[0]
            recs.append(f"Top concept cluster: {cluster['concept']} with {cluster['misses']} misses, {cluster['active_weak']} active weak, and {cluster['volatility']} stability drag.")
        if confusion_pairs:
            pair = confusion_pairs[0]
            recs.append(f"Confusion pair in rotation: {pair['pair']} has shown up {pair['count']} times. The engine will quietly feed you contrast questions for it.")
        if interference_map_rows:
            pair = interference_map_rows[0]
            recs.append(f"Interference map: {pair['pair']} is creating {pair['pressure']} pressure, so the engine will keep separating those concepts in the background.")
        if coverage_gaps:
            gap = coverage_gaps[0]
            recs.append(f"Coverage gap: {gap['kind']} {gap['unit']} is only {gap['attempted']}/{gap['available']} covered, so Smart Practice will bias into it automatically.")
        if objective_mastery_rows:
            objective = objective_mastery_rows[0]
            recs.append(f"Objective autopilot: {objective['objective_code']} is only {objective['mastery_score']}% mastered across {objective['stem_style_count']} stem styles, so Smart Practice will quietly keep feeding it.")
        if prerequisite_debt_rows:
            row = prerequisite_debt_rows[0]
            recs.append(f"Prerequisite debt: {row['kind']} {row['unit']} is carrying severity {row['severity']} and quietly dragging nearby work, so the engine will stabilize that root before chasing symptoms.")
        if knowledge_trace_rows:
            row = knowledge_trace_rows[0]
            recs.append(f"Knowledge tracing: {row['kind']} {row['unit']} is only {row['mastery_prob']}% learned with {row['uncertainty']}% uncertainty, so the engine will keep sampling it for better evidence.")
        if concept_memory_state_rows:
            row = next((item for item in concept_memory_state_rows if item['state'] in ('recognizable', 'retrievable')), concept_memory_state_rows[0])
            recs.append(f"Concept memory: {row['kind']} {row['unit']} is {row['state']}; next ramp is {row['next_ramp']}, so Smart Practice can lift it without adding clicks.")
        if wrong_answer_memory_rows:
            row = wrong_answer_memory_rows[0]
            recs.append(f"Wrong-answer memory: '{row['tempting_distractor']}' keeps competing with '{row['correct_concept']}' for {row['unit']}, so the engine will add counterexample checks.")
        if concept_half_life_rows:
            row = concept_half_life_rows[0]
            recs.append(f"Concept half-life: {row['kind']} {row['unit']} is only holding for about {row['half_life_days']} day(s), so reinforcement spacing will tighten automatically.")
        if blind_spot_rows:
            row = blind_spot_rows[0]
            recs.append(f"Blind-spot inference: {row['kind']} {row['unit']} looks under-tested with severity {row['severity']}, so Smart Practice will probe it before it becomes a visible weakness.")
        if expected_learning_gain_rows:
            row = expected_learning_gain_rows[0]
            recs.append(f"Learning-gain scheduler: Q{row['question_number']} has expected gain {row['expected_gain']}, so the engine will quietly surface it sooner than lower-payoff items.")
        if confidence_compression_rows:
            row = confidence_compression_rows[0]
            recs.append(f"Confidence compression: {row['kind']} {row['unit']} has {row['compression']} compression because {row['note'].lower()}")
        if compression_point_rows:
            row = compression_point_rows[0]
            recs.append(f"Compression point: {row['kind']} {row['unit']} drops {row['gap']} points when the abstraction level rises, so the engine will add applied checks before granting mastery.")
        if abstraction_ladder_rows:
            row = abstraction_ladder_rows[0]
            missing = ', '.join(row['missing_styles'][:3]) or 'none'
            recs.append(f"Abstraction ladder: {row['kind']} {row['unit']} is only {row['score']}% integrated and still missing styles like {missing}.")
        if recognition_retrieval_rows:
            row = recognition_retrieval_rows[0]
            recs.append(f"Recognition vs retrieval: {row['kind']} {row['unit']} has a {row['gap']} point cue-to-retrieval gap, so the engine will test it with less obvious stems.")
        if robustness_rows:
            row = robustness_rows[0]
            recs.append(f"Robustness score: {row['kind']} {row['unit']} is still {row['label'].lower()} at {row['score']}%, so the engine will keep checking it across sources and stem styles.")
        if leverage_ranking_rows:
            row = leverage_ranking_rows[0]
            recs.append(f"Leverage ranking: {row['kind']} {row['unit']} influences a lot of downstream performance ({row['leverage']} leverage), so improving it should pay back broadly.")
        if generalization_rows:
            row = generalization_rows[0]
            recs.append(f"Generalization score: {row['kind']} {row['unit']} is only {row['score']} across source and stem variation, so the engine will keep rotating scenario angles.")
        if error_boundary_rows:
            row = error_boundary_rows[0]
            recs.append(f"Error-boundary tracing: {row['kind']} {row['unit']} holds at {row['strong_style'].lower()} but breaks at {row['weak_style'].lower()} with a {row['gap']} point gap, so the engine will quietly target that transfer edge.")
        if counterfactual_distractor_rows:
            row = counterfactual_distractor_rows[0]
            recs.append(f"Counterfactual distractor memory: in {row['kind']} {row['unit']}, '{row['distractor']}' keeps beating '{row['correct']}' with pressure {row['pressure']}. The engine will quietly recycle that contrast.")
        if counterexample_training_rows:
            row = counterexample_training_rows[0]
            recs.append(f"Counterexample trainer: Q{row['question_number']} can break the wrong rule around {row['cue']}, so the engine will recycle it as a misconception check.")
        if difficulty_rows:
            row = difficulty_rows[0]
            recs.append(f"Difficulty calibration: Q{row['question_number']} is scoring {row['score']} as a {row['label'].lower()} item from real user data, so Smart Practice will pace around it more carefully.")
        if phrasing_rows:
            row = phrasing_rows[0]
            if row['label'] == 'Noisy':
                recs.append(f"Source phrasing normalization: Q{row['question_number']} is noisy at {row['score']}%, so readiness will downweight wording friction behind the scenes.")
        if misconception_fingerprints:
            row = misconception_fingerprints[0]
            recs.append(f"Misconception fingerprint: {row['fingerprint']} is recurring across {len(row['affected_units'])} unit(s), so the engine will target the underlying model rather than only the missed item.")
        if effort_efficiency_rows:
            row = effort_efficiency_rows[0]
            recs.append(f"Effort efficiency: {row['kind']} {row['unit']} is only {row['score']} efficient, so the engine will quietly revisit it until answers become cleaner and faster.")
        if decision_latency_rows:
            row = decision_latency_rows[0]
            recs.append(f"Decision latency: {row['kind']} {row['unit']} costs {row['drag']} extra seconds under confusion, so the engine will treat that as a thinking bottleneck, not just a miss.")
        if answer_latency_rows:
            row = answer_latency_rows[0]
            recs.append(f"Answer latency diagnosis: {row['kind']} {row['unit']} shows {row['label'].lower()} at pressure {row['pressure']}. {row['note']}")
        if confidence_mismatch_rows:
            row = confidence_mismatch_rows[0]
            recs.append(f"Confidence mismatch: {row['kind']} {row['unit']} has {row['sure_wrong']} confident miss(es) across {row['sure_attempts']} sure attempts. The engine will treat that confidence as fragile.")
        if cue_dependence_rows:
            row = cue_dependence_rows[0]
            recs.append(f"Cue dependence: Q{row['question_number']} is in {row['stage'].lower()} cue-support mode at {row['score']}%, so the engine will stop over-crediting easy recognition there.")
        if latent_weakness_rows:
            latent = latent_weakness_rows[0]
            recs.append(f"Latent weakness watch: Q{latent['question_number']} looks right-on-paper but still scores {latent['score']} for hidden fragility ({', '.join(latent['reasons'])}).")
        if transfer_strength_rows:
            transfer = transfer_strength_rows[0]
            recs.append(f"Transfer strength: {transfer['kind']} {transfer['unit']} is only {transfer['score']}% portable across exposures, so the engine will keep interleaving it.")
        if reinforcement_distance_rows:
            row = reinforcement_distance_rows[0]
            recs.append(f"Adaptive reinforcement distance: Q{row['question_number']} is due for another high-value check in about {row['recommended_days']} day(s), so the engine will quietly time that revisit.")
        if delayed_probe_rows:
            row = delayed_probe_rows[0]
            recs.append(f"Surprise delayed probe: Q{row['question_number']} has probe pressure {row['pressure']} after {row['days_since_seen']} day(s), so the engine will recheck it without warning.")
        if synthesis_check_rows:
            row = synthesis_check_rows[0]
            recs.append(f"Cross-objective synthesis: Q{row['question_number']} blends {row['topic_mix']} with a {row['stem_style'].lower()} stem, so it is being used as a deeper integration check.")
        if contrast_rule_rows:
            row = contrast_rule_rows[0]
            recs.append(f"Contrasting-rule library: '{row['rule']}' is active with pressure {row['pressure']}, so the engine will keep separating those ideas until the distinction is automatic.")
        if retention_stress_rows:
            row = retention_stress_rows[0]
            recs.append(f"Retention stress testing: Q{row['question_number']} is ready for a harder delayed recheck at pressure {row['pressure']}.")
        if failure_mode_rows:
            row = failure_mode_rows[0]
            recs.append(f"Failure-mode simulation: {row['kind']} {row['unit']} is mostly failing through {row['mode']}, so practice will shift toward that error shape.")
        if concept_state_rows:
            row = concept_state_rows[0]
            recs.append(f"Concept-state graph: {row['kind']} {row['unit']} is currently {row['state']}, not fully stable, so the engine will keep treating it as evidence-in-progress.")
        if source_trust_rows:
            weakest_source = source_trust_rows[0]
            if weakest_source['label'] == 'Decayed':
                recs.append(f"Source trust decay: {weakest_source['source_name']} is down to {weakest_source['trust_score']}% trust and will be weighted down behind the scenes.")
        if burnout_risk['label'] != 'Low':
            recs.append(f"Micro-burnout detector: current session risk is {burnout_risk['label'].lower()} at {burnout_risk['score']}%, so Smart Practice will flatten difficulty and bonus inserts.")
        conflict_count = sum(1 for row in source_agreement_rows if row['label'] == 'Source conflict')
        if conflict_count:
        ...
        for row in weak_domains:
            ...
            recs.append(f"{row['domain']}: readiness {row['readiness']}%, stability {row['stability']}%, heat {row['heat']}, trend {trend_word} {abs(row['trend']):.1f}, active weak {row['progress_active_weak']}, due {row['progress_due']}.")
        for row in weak_topics[:3]:
            trend_word = 'up' if row['trend'] >= 0 else 'down'
            recs.append(f"Topic '{row['topic']}' readiness {row['readiness']}%, stability {row['stability']}%, trend {trend_word} {abs(row['trend']):.1f}, active weak {row['progress_active_weak']}, due {row['progress_due']}.")
        if not recs:
        ...
        trap_word_patterns: list[TrapWordPatternRow] = [
            ...
            for trap_word, count in sorted(trap_word_counts.items(), key=lambda item: (item[1], item[0]), reverse=True)[:8]
        ]
    ...
    def compute_analytics(self, source=None) -> AnalyticsPayload:
        if source is not None:
            ...
            if signature != getattr(self, 'analytics_source_cache_key', None) or getattr(self, 'analytics_source_cache_payload', None) is None:
    ...
    def export_analytics_json(self):
        ...
        default_name = self.bank_path.with_name(f'{self.bank_path.stem}_analytics.json') if self.bank_path else Path('analytics.json')
    ...
    def open_analytics_window(self):
        ...
        tk.Label(header, text='Performance Analytics', bg=BLUE, fg='white', font=('Segoe UI', 11, 'bold'), padx=14).pack(side='left')
        ...
        tk.Button(side, text='Refresh', font=('Segoe UI', 9, 'bold'), bd=0, bg=BLUE, fg='white', padx=14, pady=8, command=self.refresh_analytics_window).pack(fill='x')
        tk.Button(side, text='Export JSON', font=('Segoe UI', 9, 'bold'), bd=1, relief='solid', bg='#f7f9fc', fg=BLUE, padx=14, pady=8, command=self.export_analytics_json).pack(fill='x', pady=(8, 0))
        notebook = ttk.Notebook(body)
        notebook.pack(fill='both', expand=True, pady=(12, 0))
        self.analytics_widgets['domain_tree'] = self._make_tree(notebook, [('domain', 'Domain', 220), ('answered', 'Answered', 75), ('correct', 'Correct', 65), ('wrong', 'Wrong', 65), ('accuracy', 'Accuracy %', 85), ('coverage', 'Coverage %', 85), ('readiness', 'Readiness', 80), ('stability', 'Stability', 80), ('trend', 'Trend', 70), ('progress_wrong', 'LT Wrong', 75), ('progress_due', 'Due', 55), ('heat', 'Heat', 65)], 'Domains')
        self.analytics_widgets['topic_tree'] = self._make_tree(notebook, [('topic', 'Topic', 260), ('seen', 'Seen', 65), ('correct', 'Correct', 70), ('wrong', 'Wrong', 70), ('accuracy', 'Accuracy %', 90), ('readiness', 'Readiness', 80), ('stability', 'Stability', 80), ('trend', 'Trend', 70), ('progress_wrong', 'LT Wrong', 75), ('progress_due', 'Due', 55)], 'Topics')
        self.analytics_widgets['mastery_tree'] = self._make_tree(notebook, [('topic', 'Topic', 240), ('new', 'New', 55), ('active_weak', 'Weak', 55), ('recovered', 'Recovered', 75), ('due', 'Due', 55), ('mastered', 'Mastered', 75), ('flagged', 'Flagged', 65), ('readiness', 'Readiness', 80)], 'Mastery Map')
        recent_frame = tk.Frame(notebook, bg=CARD)
        notebook.add(recent_frame, text='Hot signals')
        txt = tk.Text(recent_frame, wrap='word', font=('Segoe UI', 10), bg=CARD, fg=TEXT, relief='flat', padx=10, pady=10)
        ...
        patterns_text = tk.Text(patterns_frame, wrap='word', font=('Segoe UI', 10), bg=CARD, fg=TEXT, relief='flat', padx=10, pady=10)
    ...
    def refresh_analytics_window(self):
        ...
        for row in analytics['domains']:
            self.analytics_widgets['domain_tree'].insert('', 'end', values=(row['domain'], row['answered'], row['correct'], row['wrong'], row['accuracy'], row['coverage'], row['readiness'], row['stability'], row['trend'], row['progress_wrong'], row['progress_due'], row['heat']))
        for row in analytics['topics'][:150]:
            self.analytics_widgets['topic_tree'].insert('', 'end', values=(row['topic'], row['seen'], row['correct'], row['wrong'], row['accuracy'], row['readiness'], row['stability'], row['trend'], row['progress_wrong'], row['progress_due']))
        for row in analytics.get('topic_mastery_map', [])[:150]:
            self.analytics_widgets['mastery_tree'].insert('', 'end', values=(row['topic'], row['new'], row['active_weak'], row['recovered'], row['due'], row['mastered'], row['flagged'], row['readiness']))
        ...
        txt.insert(tk.END, f"Confidence honesty: {prediction.get('confidence_honesty', 0.0)}%    Readiness floor: {prediction.get('readiness_floor', 0.0)}%\n")
        ...
        for row in analytics.get('volatile_questions', []):
            txt.insert(tk.END, f"Q{row['question_number']}  {row['domain']}  {row['label']} volatility {row['score']}  {row['flips']} flips across {row['attempts']} tries\n")
        txt.insert(tk.END, '\nConcept mistake clusters\n\n')
        for row in analytics.get('concept_clusters', []):
            txt.insert(tk.END, f"{row['concept']}  severity {row['severity']}  misses {row['misses']}  weak {row['active_weak']}  due {row['due']}  top miss {row['top_miss_reason'] or 'n/a'}\n")
        txt.insert(tk.END, '\nCoverage gap detector\n\n')
        for row in analytics.get('coverage_gaps', [])[:8]:
            txt.insert(tk.END, f"{row['kind']} {row['unit']}  severity {row['severity']}  covered {row['attempted']}/{row['available']}  accuracy {row['accuracy']}%\n")
        txt.insert(tk.END, '\nObjective-code mastery autopilot\n\n')
        for row in analytics.get('objective_mastery', [])[:8]:
            txt.insert(tk.END, f"{row['objective_code']}: mastery {row['mastery_score']}%  attempted {row['attempted']}/{row['available']}  stems {row['stem_style_count']}  sources {row['source_count']}  due {row['due']}\n")
        txt.insert(tk.END, '\nConfidence compression\n\n')
        for row in analytics.get('confidence_compression', [])[:8]:
            txt.insert(tk.END, f"{row['kind']} {row['unit']}: compression {row['compression']}  fragile {row['fragile_correct']}/{row['correct_total']}  stability {row['stability']}\n")
        txt.insert(tk.END, '\nAbstraction ladder\n\n')
        for row in analytics.get('abstraction_ladder', [])[:8]:
            missing = ', '.join(row['missing_styles'][:3]) or 'none'
            txt.insert(tk.END, f"{row['kind']} {row['unit']}: {row['label']}  {row['score']}%  rungs {row['rung_count']}/{row['available_style_count']}  missing {missing}\n")
        txt.insert(tk.END, '\nError-boundary tracing\n\n')
        for row in analytics.get('error_boundaries', [])[:8]:
            txt.insert(tk.END, f"{row['kind']} {row['unit']}: weak {row['weak_style']} {row['weak_accuracy']}% vs strong {row['strong_style']} {row['strong_accuracy']}%  gap {row['gap']} across {row['attempts']} tries\n")
        txt.insert(tk.END, '\nCounterfactual distractor memory\n\n')
        for row in analytics.get('counterfactual_distractors', [])[:8]:
            txt.insert(tk.END, f"{row['kind']} {row['unit']}: '{row['distractor']}' keeps beating '{row['correct']}'  pressure {row['pressure']}  count {row['count']}\n")
        txt.insert(tk.END, '\nConcept memory state\n\n')
        for row in analytics.get('concept_memory_states', [])[:10]:
            txt.insert(tk.END, f"{row['kind']} {row['unit']}: {row['state']}  evidence {row['evidence_count']}  confidence {row['confidence_quality']}%  transfer {row['transfer_evidence']}  next {row['next_ramp']}\n")
        txt.insert(tk.END, '\nWrong-answer memory\n\n')
        for row in analytics.get('wrong_answer_memory', [])[:8]:
            txt.insert(tk.END, f"{row['kind']} {row['unit']}: '{row['tempting_distractor']}' vs '{row['correct_concept']}'  pressure {row['pressure']}  count {row['count']}  examples {row['example_question_numbers'][:4]}\n")
        txt.insert(tk.END, '\nDifficulty calibration\n\n')
        for row in analytics.get('difficulty_calibration', [])[:8]:
            txt.insert(tk.END, f"Q{row['question_number']}  {row['label']}  score {row['score']}  wrong {row['wrong_rate']}%  fragile {row['fragile_rate']}%  volatility {row['volatility']}\n")
        ...
        txt.insert(tk.END, f"{burnout.get('label', 'Low')}  {burnout.get('score', 0.0)}%  accuracy drop {burnout.get('accuracy_drop', 0.0)}  response drag {burnout.get('response_drag', 0.0)}s  fragile {burnout.get('fragile_rate', 0.0)}%\n")
        txt.insert(tk.END, '\nSource agreement layer\n\n')
        for row in analytics.get('source_agreement', [])[:8]:
            txt.insert(tk.END, f"Q{row['question_number']}  {row['label']}  trust {round(row['score'] * 100)}%  source {row['source_name']}\n")
        txt.insert(tk.END, '\nDynamic source trust decay\n\n')
        for row in analytics.get('source_trust', [])[:8]:
            txt.insert(tk.END, f"{row['source_name']}: {row['label']}  trust {row['trust_score']}%  decay {row['decay']}  conflicts {row['conflict_count']}  issues {row['issue_count']}\n")
        txt.insert(tk.END, '\nLatent weakness detector\n\n')
        for row in analytics.get('latent_weakness', [])[:8]:
            txt.insert(tk.END, f"Q{row['question_number']}  {row['score']}  {row['confidence_signal']}  stability {row['stability']}  {', '.join(row['reasons'])}\n")
        txt.insert(tk.END, '\nTransfer-strength scoring\n\n')
        for row in analytics.get('transfer_strength', [])[:8]:
            txt.insert(tk.END, f"{row['kind']} {row['unit']}: {row['label']}  {row['score']}%  exposure {row['exposure']}  sources {row['source_count']}  stems {row['stem_style_count']}  due {row['due']}\n")
        ...
        txt.insert(tk.END, '- Readiness rewards accuracy, coverage, confidence, and recovery while penalizing active weak and due drag.\n')
        txt.insert(tk.END, '- Decision quality rewards correct answers with honest confidence and penalizes overconfident misses.\n')
        ...
        if interference_rows:
            for row in interference_rows:
                patterns_text.insert(tk.END, f"{row['pair']}: pressure {row['pressure']} across {row['count']} misses. {row['action']}\n")
        else:
        ...
        if error_boundary_rows:
            for row in error_boundary_rows:
                patterns_text.insert(tk.END, f"{row['kind']} {row['unit']}: {row['weak_style']} is trailing {row['strong_style']} by {row['gap']} points. {row['note']}\n")
        else:
        ...
        if counterfactual_rows:
            for row in counterfactual_rows:
                patterns_text.insert(tk.END, f"{row['kind']} {row['unit']}: '{row['distractor']}' beats '{row['correct']}' {row['count']} times. {row['note']}\n")
        else:
        ...
        if memory_rows:
            for row in memory_rows:
                patterns_text.insert(tk.END, f"{row['kind']} {row['unit']}: '{row['tempting_distractor']}' keeps competing with '{row['correct_concept']}' at pressure {row['pressure']}. {row['note']}\n")
        else:
            patterns_text.insert(tk.END, 'No concept-level tempting distractor pressure detected yet.\n')
        patterns_text.insert(tk.END, '\nHow to use this\n\n')
        patterns_text.insert(tk.END, '- If guessed answers are often correct, treat them as unstable knowledge and review them before trusting readiness.\n')
        patterns_text.insert(tk.END, '- Repeated miss reasons usually point to a habit problem, not just a content gap.\n')
        patterns_text.insert(tk.END, '- Trap-word counts help you slow down before qualifier, exception, and timing questions.\n')
        patterns_text.insert(tk.END, '- Volatile questions are the ones flipping between right and wrong; review the rule behind them, not just the key.\n')
        patterns_text.insert(tk.END, '- Stability scores reward concepts that stay correct over time, not just one-off wins.\n')
        patterns_text.insert(tk.END, '- Concept clusters and remediation cards show where repeated misses belong to the same idea, not separate questions.\n')
